    ...
    if (session.currentFrame){
        if (session.currentFrame.camera){
            ...
            for (int i = 0; i < mats.size(); i++){
                ...
                ofVec3f test = ARCommon::getAnchorXYZ(mat);
...
void ofApp::touchDown(ofTouchEventArgs &touch){
    ...
    cout << processor->anchorController->getNumAnchors() << endl;
    cout << processor->getCameraPosition().x <<" " << processor->getCameraPosition().y << " " << processor->getCameraPosition().z << endl;
